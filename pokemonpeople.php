<?php
// filepath: c:\git\pokemon-database\pokemonpeople.php
...
if ($_POST) {
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            case 'add':
                try {
                    $stmt = $pdo->prepare("INSERT INTO Pokemon_People (pokeID, peopleID, pokeNickname, caughtDate, caughtLocation) VALUES (?, ?, ?, ?, ?)");
            ...
            case 'update':
                try {
                    $stmt = $pdo->prepare("UPDATE Pokemon_People SET pokeID = ?, peopleID = ?, pokeNickname = ?, caughtDate = ?, caughtLocation = ? WHERE pokeID = ? AND peopleID = ?");
...
try {
    $stmt = $pdo->query("
        SELECT pp.pokeID, pp.peopleID, pp.pokeNickname, pp.caughtDate, pp.caughtLocation,
...
<head>
  <title>Pokemon Management Database - Owned Pokemon</title>
  ...
  <script language="JavaScript">
    ...
    function updatePokePers(pokeID, peopleID, pokeName, peopleName, pokeNickname, caughtDate, caughtLocation) { 
      showform('update'); 
      ...
      document.getElementById('updateCaughtAt').value = caughtLocation;
      document.getElementById('updateDisplayInfo').innerHTML = 
        '<strong>Current:</strong> ' + pokeName + ' owned by ' + peopleName;
    }
    function deletePokePers(pokeID, peopleID, pokeName, peopleName, pokeNickname, caughtLocation) { 
      showform('delete'); 
      ...
      document.getElementById('deleteCaughtLocationDisplay').textContent = caughtLocation;
    }
        ...
        <div id="browse">
          ...
          <table border="1" cellpadding="5" id="centerMargin">
            ...
            <tr>
              ...
              <td><?php echo htmlspecialchars($owned['caughtLocation']); ?></td>
              <td><a href="#" onClick="updatePokePers(<?php echo $owned['pokeID']; ?>, <?php echo $owned['peopleID']; ?>, '<?php echo htmlspecialchars($owned['pokeName']); ?>', '<?php echo htmlspecialchars($owned['peopleName']); ?>', '<?php echo htmlspecialchars($owned['pokeNickname'] ?? ''); ?>', '<?php echo $owned['caughtDate']; ?>', '<?php echo htmlspecialchars($owned['caughtLocation']); ?>')">Edit</a></td>
              <td><a href="#" onclick="deletePokePers(<?php echo $owned['pokeID']; ?>, <?php echo $owned['peopleID']; ?>, '<?php echo htmlspecialchars($owned['pokeName']); ?>', '<?php echo htmlspecialchars($owned['peopleName']); ?>', '<?php echo htmlspecialchars($owned['pokeNickname'] ?? ''); ?>', '<?php echo htmlspecialchars($owned['caughtLocation']); ?>')">Delete</a></td>
            </tr>
            <?php endforeach; ?>
          </table>
          <p>&nbsp;</p>
        ...
        <div id="update" style="display: none">
          <form method="POST" id="updatePokePers">
            ...
            <fieldset class="fields">
              ...
              <br>
              <label> Nickname </label> <input type="text" name="pokeNickname" id="updatePokeNickname" style="margin-bottom: 10px">
              <br>
              <label>Caught Date</label> <input type="date" name="caughtDate" id="updateCaughtDate" style="margin-bottom: 10px">
              <br>
              <label> Caught Location </label> <input type="text" name="caughtAt" id="updateCaughtAt" style="margin-bottom: 10px">
              <br><br>
            </fieldset>
            <p id="center">
