<html>

<head>
  <title>Pokemon Management Database - Pokemon Types</title>
  <link href="main.css" rel="stylesheet" type="text/css" />
  <script language="JavaScript">
    function showform(dowhat) {
      /*
      * four DIVS: browse, insert, update, delete
      * this function sets one visible the others not
      */
      if (dowhat == 'insert') {
        document.getElementById('browse').style.display = 'none';
        document.getElementById('insert').style.display = 'block';
        document.getElementById('update').style.display = 'none';
        document.getElementById('delete').style.display = 'none';
      }
      else if (dowhat == 'update') {
        document.getElementById('browse').style.display = 'none';
        document.getElementById('insert').style.display = 'none';
        document.getElementById('update').style.display = 'block';
        document.getElementById('delete').style.display = 'none';
      }
      else if (dowhat == 'delete') {
        document.getElementById('browse').style.display = 'none';
        document.getElementById('insert').style.display = 'none';
        document.getElementById('update').style.display = 'none';
        document.getElementById('delete').style.display = 'block';
      }
      else if (dowhat == 'all') {
        document.getElementById('browse').style.display = 'block';
        document.getElementById('insert').style.display = 'block';
        document.getElementById('update').style.display = 'block';
        document.getElementById('delete').style.display = 'block';
      }
      else { //by default display browse
        document.getElementById('browse').style.display = 'block';
        document.getElementById('insert').style.display = 'none';
        document.getElementById('update').style.display = 'none';
        document.getElementById('delete').style.display = 'none';
      }
    }
    function newPokeType() { showform('insert'); }
    function updatePokeType(PokeTypeID) { showform('update'); }
    function deletePokeType(PokeTypeID) { showform('delete'); }
    function showAll() { showform('all'); }
    function browsePokeType() { showform('browse'); }
  </script>
</head>

<body>
  <header>
    <h1>Pokemon Management System Database</h1>
  </header>
  <nav>
    <a href="index.html">Home</a>
    <a href="pokemon.html">Pokemon</a>
    <a href="people.html">People</a>
    <a href="affiliations.html">Affiliations</a>
    <a href="types.html">Types</a>
    <a href="battles.html">Battles</a>
    <br><br>
    <a href="pokemontypes.html">Pokemon Types</a>
    <a href="pokemonpeople.html">Owned Pokemon</a>
    <a href="peoplebattles.html">Player Battles</a>
  </nav>
  <main>
    <section>
      <h2>Pokemon Typing</h2>
      <article id="pokemontypes">
        <p id="center">
          List of Pokemon and their associated typing.
        </p>

        <div id="browse">
          <p id="center"><a href="#" onClick="showAll()">Display all forms</a> | <a href="#"
              onClick="browsePokeType()">Hide forms</a><br>
            <a href="#" onClick="newPokeType()">New</a>
          </p>
          <table border="1" cellpadding="5" id="centerMargin">
            <tr>
              <th>Pokemon</th>
              <th>Type</th>
              <th>Edit Entry</th>
              <th>Delete Entry</th>
            </tr>
            <tr>
              <td>Pikachu</td>
              <td>Electric</td>
              <td><a href="#" onClick="updatePokeType('this.PokeTypeID')">Edit</a></td>
              <td><a href="#" onclick="deletePokeType('this.PokeTypeID')">Delete</a></td>
            </tr>
            <tr>
              <td>Charizard</td>
              <td>Fire</td>
              <td><a href="#" onClick="updatePokeType('this.PokeTypeID')">Edit</a></td>
              <td><a href="#" onclick="deletePokeType('this.PokeTypeID')">Delete</a></td>
            </tr>
            <tr>
              <td>Budew</td>
              <td>Grass</td>
              <td><a href="#" onClick="updatePokeType('this.PokeTypeID')">Edit</a></td>
              <td><a href="#" onclick="deletePokeType('this.PokeTypeID')">Delete</a></td>
            </tr>
            <tr>
              <td>Mewtwo</td>
              <td>Psychic</td>
              <td><a href="#" onClick="updatePokeType('this.PokeTypeID')">Edit</a></td>
              <td><a href="#" onclick="deletePokeType('this.PokeTypeID')">Delete</a></td>
            </tr>
            <tr>
              <td>Arceus</td>
              <td>Normal</td>
              <td><a href="#" onClick="updatePokeType('this.PokeTypeID')">Edit</a></td>
              <td><a href="#" onclick="deletePokeType('this.PokeTypeID')">Delete</a></td>
            </tr>
          </table>
          <p>&nbsp;</p>
        </div> <!-- browse -->

        <div id="insert" style="display: none">
          <form method="POST" id="addPokeType">
            <legend><strong>Add a Pokemon and their typing</strong></legend>
            <fieldset class="fields">
              <label>* Pokemon </label> <select name="Pokemon" style="margin-bottom: 10px" required>
                <option value="0">&nbsp;</option>
                <option value="1">Pikachu</option>
                <option value="2">Charizard</option>
                <option value="3">Budew</option>
                <option value="4">Mewtwo</option>
                <option value="5">Arceus</option>
              </select>
              <br>
              <label>* Type </label> <select name="Type" style="margin-bottom: 10px" required>
                <option value="0">&nbsp;</option>
                <option value="1">Fire</option>
                <option value="2">Water</option>
                <option value="3">Grass</option>
                <option value="4">Ghost</option>
                <option value="5">Electric</option>
                <option value="6">Psychic</option>
                <option value="7">Normal</option>
              </select>
              <br>
              <button type="submit" id="addPokeType">Add Typing to Pokemon</button>
              <button type="button" onClick="browsePokeType()">Cancel</button>
              </p>
          </form>
        </div><!-- insert -->

        <p>&nbsp;</p>
        <div id="update" style="display: none">
          <form method="POST" id="updatePokeType">
            <legend><strong>Update Pokemon and their Typing</strong></legend>
            <fieldset class="fields">
              <label> Pokemon </label> <select name="Pokemon" style="margin-bottom: 10px">
                <option value="0">Pikachu</option>
                <option value="1">Charizard</option>
                <option value="2">Budew</option>
                <option value="3">Mewtwo</option>
                <option value="4">Arceus</option>
              </select>
              <br>
              <label> Type </label> <select name="Type" style="margin-bottom: 10px">
                <option value="0">Electric</option>
                <option value="1">Fire</option>
                <option value="2">Water</option>
                <option value="3">Grass</option>
                <option value="4">Ghost</option>
                <option value="5">Psychic</option>
                <option value="6">Normal</option>
              </select>
              <br><br>
            </fieldset>
            <p id="center">
              <button type="submit" id="updateSavePokeType">Save Pokemon Updates</button>
              <button type="button" onClick="browsePokeType()">Cancel</button>
            </p>
          </form>
        </div><!-- update -->

        <p>&nbsp;</p>
        <div id="delete" style="display: none">
          <form method="POST" id="deletePokeType">
            <legend><strong>Delete Pokemon Typing</strong></legend>
            <fieldset class="fields">
              <p>Are you sure you want to delete this Pokemon Type pairing from the database?</p>
              <input type="hidden" name="PokeTypeID" id="deletePokeTypeID" value="1">
              <label><strong>Pokemon: </strong></label> Pikachu
              <br>
              <label><strong>Type: </strong></label> Electric
              <br><br>
            </fieldset>
            <p id="center">
              <button type="submit" id="deletePokeType">Delete Pokemon Typing</button>
              <button type="button" onClick="browsePokeType()">Cancel</button>
            </p>
          </form>
        </div><!-- delete -->
      </article>
    </section>
  </main>
  <footer>© 2025 - Elena Parapounsky & Amy Xu</footer>
</body>

</html>